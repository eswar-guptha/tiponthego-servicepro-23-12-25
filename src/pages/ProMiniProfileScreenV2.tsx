 ...
 *
 * Enhancement (Fix): Add vertical scrollbar for the sheet body
 *  - Sheet is height-limited with scrollable content
 *  - Custom right-side scroll indicator (matches screenshot intent)
 */
...
// Custom scrollbar math for the scroll indicator
function computeThumb(
  ...
  // thumb size: min 18%, max 55% depending on content length
...
function runDevChecks() {
  try {
    ...
    const t = computeThumb(50, 300, 100);
...
export default function ProMiniProfileScreenV2() {
  ...
  function openSheet(p: Pro) {
    setSelected(p);
    // reset scroll indicator shortly after mount
    setTimeout(() => {
  ...
  // Keep scrollbar indicator in sync
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;
        ...
        {selected && (
          ...
          >
            ...
            >
              ...
              {/* Header row (fixed) */}
              <div className="px-4 pt-3 pb-4 border-b bg-white">
              ...
              {/* Scrollable body with indicator */}
              <div className="relative">
                <div
                  ref={scrollRef}
                  className="px-4 py-4 space-y-4 max-h-[66vh] overflow-y-auto pr-5"
                ...
                >
                  ...
                  {/* Spacer to ensure CTA isn't cramped when scrolled to bottom */}
                  <div className="h-6" />
                </div>

                {/* Right-side scroll indicator (visual) */}
                <div className="pointer-events-none absolute top-3 bottom-3 right-3 w-[6px] rounded-full bg-gray-200/70">
                  <div
