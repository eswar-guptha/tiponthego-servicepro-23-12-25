 ...
 *
 * Added:
 * - X1.2 Tip Receipt (full screen)
 * - X2.2 QR Scan (camera mock + manual code)
 */
...
function buildQrDirectory(pros: Pro[]): QrPayload[] {
  return pros.map((p) => ({ code: `TIP-${p.id.toUpperCase()}`, proId: p.id }));
}

function resolveProFromTipCode(
  code: string,
  pros: Pro[],
  directory: QrPayload[]
) {
  const trimmed = code.trim().toUpperCase();
  const hit = directory.find((q) => q.code === trimmed);
  if (!hit) return null;
  return pros.find((p) => p.id === hit.proId) ?? null;
}
...
function runDevChecks() {
  try {
    ...
    // QR resolver checks
    const dir = buildQrDirectory(PROS);
    console.assert(
      dir.length === PROS.length,
      "QR directory should include all pros"
    );
    console.assert(
      resolveProFromTipCode("TIP-P1", PROS, dir)?.id === "p1",
      "Should resolve TIP-P1 to p1"
    );
    console.assert(
      resolveProFromTipCode(" tip-p2 ", PROS, dir)?.id === "p2",
      "Should be case/space insensitive"
    );
    console.assert(
      resolveProFromTipCode("TIP-UNKNOWN", PROS, dir) === null,
...
export default function TipNowScreenV2() {
...
function ReceiptScreen({
  ...
  async function shareReceipt() {
    try {
      // @ts-expect-error - Web Share API is not always typed
      if (navigator.share) {
        // @ts-expect-error
        await navigator.share({ title: "Tip Receipt", text: receiptText });
        return;
      }
    } catch {
      // ignore
...
function QrScanScreen({
  ...
  const qrDirectory: QrPayload[] = useMemo(() => buildQrDirectory(PROS), []);
  const hintCodes = useMemo(
    () => qrDirectory.slice(0, 3).map((q) => q.code),
    [qrDirectory]
  );
  ...
  async function simulateScan() {
    ...
    const pick = PROS[Math.floor(Math.random() * PROS.length)];
    setStatus("found");
  ...
  function submitManual() {
    const pro = resolveProFromTipCode(manualCode, PROS, qrDirectory);
    if (!pro) {
      setStatus("invalid");
      return;
    }
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 z-[75] bg-gradient-to-br from-blue-600 to-cyan-500"
    >
      <div className="max-w-md mx-auto min-h-screen flex flex-col">
        <div className="px-4 pt-6 pb-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-white font-semibold text-lg">Scan QR</div>
            <button
              onClick={onClose}
              className="w-11 h-11 rounded-2xl bg-white/15 border border-white/20 text-white flex items-center justify-center hover:bg-white/20"
              aria-label="Close"
          ...
          <div className="mt-4 bg-white/15 border border-white/20 rounded-3xl p-2 flex gap-2">
            <button
              onClick={() => {
                setMode("camera");
                setStatus("idle");
        ...
        <div className="px-4 pb-6 flex-1">
          {mode === "camera" ? (
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="text-sm text-gray-500 mt-1">
                    We’ll detect the pro instantly and open the tip flow.
                  </div>
                </div>
                <button
                ...
                >
                  <Flashlight
                    className={`w-4 h-4 ${
                      flash ? "text-yellow-700" : "text-gray-700"
                    }`}
                  />
                </button>
              </div>

              <div className="mt-4 relative h-[360px] rounded-3xl overflow-hidden bg-gradient-to-br from-gray-900 to-gray-800">
                <div
                  className="absolute inset-0 opacity-30"
                  style={{
                    backgroundImage:
                      "radial-gradient(circle at 30% 30%, rgba(255,255,255,.25), transparent 55%), radial-gradient(circle at 70% 70%, rgba(255,255,255,.18), transparent 55%)",
                  }}
                />

                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="relative w-[78%] max-w-[320px] aspect-square rounded-3xl border border-white/40 bg-white/5">
                    <div className="absolute -top-2 -left-2 w-10 h-10 border-l-4 border-t-4 border-white rounded-tl-3xl" />
                    <div className="absolute -top-2 -right-2 w-10 h-10 border-r-4 border-t-4 border-white rounded-tr-3xl" />
                    <div className="absolute -bottom-2 -left-2 w-10 h-10 border-l-4 border-b-4 border-white rounded-bl-3xl" />
                    <div className="absolute -bottom-2 -right-2 w-10 h-10 border-r-4 border-b-4 border-white rounded-br-3xl" />

                    <motion.div
                      className="absolute left-4 right-4 h-0.5 bg-emerald-300/90 shadow-[0_0_18px_rgba(110,231,183,0.85)]"
                      initial={{ top: 12, opacity: 0.8 }}
                      animate={{ top: [12, 260, 12], opacity: [0.8, 1, 0.8] }}
                      transition={{
                        duration: 2.2,
                        repeat: Infinity,
                        ease: "easeInOut",
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-white/85 text-sm text-center px-6">
                        <div className="font-semibold">
                          Align QR inside the frame
                        </div>
                        <div className="text-white/60 text-xs mt-1">
                          Auto-detect · Fast & secure
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="absolute top-4 left-4">
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/15 border border-white/20 text-white text-xs">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-4">
              <div className="font-semibold text-lg">Enter tip code</div>
              <div className="text-sm text-gray-500 mt-1">
                Type the code shown near the pro’s QR.
              </div>
              ...
              {status === "invalid" && (
                <div className="mt-3 text-sm text-red-600 bg-red-50 border border-red-100 rounded-2xl p-3">
                  Invalid code. Try:{" "}
                  <span className="font-semibold">{hintCodes.join(", ")}</span>
                </div>
              )}
